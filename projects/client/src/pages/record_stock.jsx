import {
    ...
    FormControl, FormLabel, Input, useToast, Heading, IconButton, Tooltip, Table, Tr, Td, Thead, Tbody, Select, FormHelperText, Grid, Textarea, border
...
export default function Record() {
    ...
    const { isOpen : isOpenDesc, onOpen : onOpenDesc, onClose : onCloseDesc } = useDisclosure();
    ...
    const editStock = (id) => {
        axiosInstance.get(`/product/productbybranch/${id}`).then((res) => {
            setProductData([...res.data.result])
        })
    }

    async function fetchRecord(id) {
        await axiosInstance.get(`/stock/fetchRecordById/${id}`).then((res) => {
    ...
    const formik = useFormik({
        ...
        validationSchema : Yup.object().shape({
            ...
            stock: Yup.number("Stock must be a number").required("Stock must be filled"),    
    ...
    return (
        <>
            <Flex w='100%' bg='gray.100'>
                        ...
                        <Flex w={['70%', '80%', '100%']} h='100%' overflow='auto' sx={webkit} bg='white' borderRadius={10}>
                            <Stack w={'80%'} m='0 auto' spacing={3} py={4} >
                                ...
                                >
                                    ...
                                    <Table fontSize={['md', 'lg', 'xl']} w='700px' margin={0} padding={0}>
                                    <Thead borderBottom={'2px solid #2C3639'}>
                                    <Tr>
                                    <Grid templateColumns={'1fr 3fr 2fr 2fr 2fr'} placeItems={"center"} >
                                        <Td textAlign={'center'} border={0}>No</Td>
                                        <Td textAlign={'center'} border={0}>Product Name</Td>
                                        <Td textAlign={'center'} border={0}>Stock Before</Td>
                                        <Td textAlign={'center'} border={0}>Stock After</Td>
                                        <Td textAlign={'center'} border={0}>Description</Td>
                                    </Grid>
                                        </Tr>
                                    </Thead>
                                    <Tbody>
                                    {recordStock?.map((val, idx) => {
                                        return <Tr key={idx} w='100%' p="3px 2px 2px 20px" borderBottom='1px solid #2C3639'
                                                 _hover={{
                                                    backgroundColor: 'gray.200'
                                                }} >
                                            <Grid templateColumns={'1fr 3fr 2fr 2fr 2fr'} placeItems={"center"}>    
                                                ...
                                                <Flex w="100px" justify='space-around'>
                                                    <Tooltip label='Description' placement='top-start'>
                                                        <IconButton // edit button
                                                            ...
                                                            bg='none' 
                                                            onClick={ () => detailDesc( {descProd : val.desc, nameProd : val.Product?.name})}
                                                        />
                                                    </Tooltip>
                                                </Flex>
                                                </Td>
                ...
                <Select  name='idProduct' placeholder='Select' onChange={(e) => formik.setFieldValue("idProduct", e.target.value)}>
                {
                      productData.map((p)=> {
                        return (
              ...
              />
               <FormHelperText  w={"inherit"} marginTop={"5px"} color={"red.500"} fontSize={"9px"} >
                    {formik.errors.desc}
                                        ...
                                        <ModalFooter>
              <Button colorScheme='teal' color={'white'} bg='#2C3639'  sx={{  _hover: { bg: '#3F4E4F' }
